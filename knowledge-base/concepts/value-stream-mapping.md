---
type: concept
tags: [concept]
---

# Value Stream Mapping

A Lean technique for visualizing the end-to-end flow of value creation from customer need to delivery. Maps steps, inputs, outputs, roles, wait times, and types of work. Always done **[Outside-In Thinking](outside-in.md)** — starting from the customer. Foundation of the Value Stream Assessment Template.

---

## Books that address this concept

- [Lean Thinking: Banish Waste and Create Wealth in Your Corporation](../books/womack-jones-lean-thinking.md)
- [The Toyota Production System: Beyond Large-Scale Production](../books/ohno-the-toyota-production-system.md)
- [Kanban: Successful Evolutionary Change for Your Technology Business](../books/anderson-kanban-successful-evolutionary-change-for-your-technology-bu.md)
- [The DevOps Handbook: How to Create World-Class Agility, Reliability, and Security in Technology Organizations](../books/kim-the-devops-handbook-how-to-create-world-class-agility-reliab.md)

---

## Connection to our work

The core methodology of Part I of the [AI-First Value Stream Assessment](../../AI-First Value Stream Assessment Template.md). Section 2 is the mapping table.
