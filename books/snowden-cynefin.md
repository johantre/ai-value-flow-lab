...
status: reference
rating: not rated
tags: [thinking, complexity, organizational, change]
source: reference
---
...
```mermaid
flowchart TB
    subgraph COMP["Complicated — Sense · Analyze · Respond"]
        direction TB
        C1["Expertise vereist"]
        C2["Meerdere juiste antwoorden"]
        C3["Good practices"]
    end

    subgraph CPLX["Complex — Probe · Sense · Respond"]
        direction TB
        X1["Oorzaak alleen achteraf zichtbaar"]
        X2["Safe-to-fail experimenten"]
        X3["Emergente praktijken"]
    end

    subgraph CLR["Clear/Simple/Order — Sense · Categorize · Respond"]
        direction TB
        L1["Oorzaak-gevolg evident"]
        L2["Best practices"]
        L3["Gevaar: klif naar Chaos"]
    end

    subgraph CHO["Chaotic — Act · Sense · Respond"]
        direction TB
        H1["Geen oorzaak-gevolg zichtbaar"]
        H2["Handelen om te stabiliseren"]
        H3["Gevaar: macht door crisis"]
    end

    DIS["Disorder\nWeet niet welk domein"]

    DIS -.-> COMP
    DIS -.-> CPLX
    DIS -.-> CLR
    DIS -.-> CHO
    CLR -->|"klif bij complacency"| CHO
    CPLX -->|"bij voldoende begrip"| COMP
```
