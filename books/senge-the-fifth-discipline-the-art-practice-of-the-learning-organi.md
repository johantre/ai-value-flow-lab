...
flowchart LR
    A1["Partij A handelt"]
    B1["Partij B voelt bedreiging"]
    B2["Partij B escaleert"]
    A2["Partij A voelt bedreiging"]

    A1 -->|"bedreigt B"| B1
    B1 --> B2
    B2 -->|"bedreigt A"| A2
    A2 -->|"A escaleert verder"| A1
```
...
flowchart TB
    ...
    RA -->|"vergroot"| SA
    RB -->|"vermindert"| SB
    SA -->|"rechtvaardigt meer"| ALLOC
    SB -->|"rechtvaardigt minder"| ALLOC
```
...
flowchart TB
    P2["Probleem"]
    QF["Snelle fix"]
    SR["Korte-termijn verlichting"]
    SE["Vertraagde bijwerking"]

    P2 -->|"drijft naar"| QF
    QF -->|"verlicht"| SR
    SR -.->|"maskeert"| P2
    QF -->|"veroorzaakt"| SE
    SE -->|"verergert op lange termijn"| P2
```
...
flowchart TB
    AS["Succes van A"]
    AH["Actie A voor eigen succes"]
    UB["Onbedoeld effect op B"]
    BS["Succes van B"]
    BH["Actie B voor eigen succes"]
    UA["Onbedoeld effect op A"]
