...
```mermaid
flowchart TD
    A([❶ Consent to purpose]) --> B([❷ Present proposal])
    B --> C([❸ Understand proposal])
    C --> D([❹ Brief response])
    D --> E{❺ Objections?}
    E -->|geen bezwaren| H([❽ Celebrate])
    E -->|bezwaar| F{❻ Test argument}
    F -->|niet gekwalificeerd| E
    F -->|gekwalificeerd| G([❼ Resolve objection])
    G --> E
    H --> I([❾ Consider concerns])
```
